...
// GoalSeekAI 함수는 주어진 매개 변수 값을 이용해 계산 결과를 디스플레이합니다.
    ...
    const AJ7GoalSeekAL7 = (r6, u7, ak7,al7, ai7) => {
        if (r6 === "" || u7 === "") {
            ...
            return 1.3 * Math.pow((Math.pow(ak7 * al7, 5) / Math.pow(ak7 + al7, 2)), (1 / 8)) / ai7;

        }
    }

    const AJ7GoalSeekAL7_2 = (r6, u7, ak7,al7, ai7) => {
        let precision = 0.0001;  // 원하는 정확도 설정
        let stepSize = 0.1;  // "an" 값 조정 단계 설정
        let al = al7;  // 초기 "an" 값 설정
        let aj = AJ7GoalSeekAL7(r6, u7, ak7, al, ai7);  // 초기 "am" 값 계산

        while (Math.abs(al - 1) > precision) { // "am" 값이 1에 가까워지는 동안 "an" 값 조정
            al += stepSize; // "an" 값 조정
            aj = AJ7GoalSeekAL7(r6, u7, ak7,al, ai7);  // 새로운 "am" 값 계산
        }

        return al; // 조정된 "an" 값 반환
    }
    ...
    const duct_cal03 = (state) => {
        ...
        if (state.enterChecked === true) {
            ...
            if (state.ducts === "사각덕트") {
                ...
                al7 = AJ7GoalSeekAL7_2(r6, u7,al7, ak7, ai7);
