...
const DuctInput = () => {
    ...
    const handleChange = useCallback((e) => {
        ...
        const onSwitchStates = {
            onSwitchTrueAndInsertSpace: setState({
                ...state,
                onSwitch: true,
                [eTName]: ""
            }),
            onSwitchTrueAndInsert1: setState({
                ...state,
                onSwitch: true,
                [eTName]: 1
            }),
            onSwitchTrueAndInsert10: setState({
                ...state,
                onSwitch: true,
                [eTName]: 10
            }),
        ...
        <div className={"DuctInputClass"} onKeyDown={handleKeyDown}>
            ...
            <form className={"form-container"}>
                <div className={"form-field"}>
                    ...
                    <span>재료조도</span>
                    <input ref={useRefs.materialRoughness}
                           className={"DuctInput"}
                           value={state.materialRoughness}
                ...
                <div className="form-field">
                    {/*0미만 x 콤마(,) x 닷(.)*/}
                    <span>최소크기</span>
                    <input className={"DuctInput"} value={state.minSize}
                           onChange={handleChange} name={"minSize"}
                           onKeyDown={handleChange} ref={useRefs.minSize}
                           required/>
                </div>
                <div className="form-field">
                    <p className="no-wrap">
            ...
            <div className={"checkBoxing"}>
                <div className="checkbox-wrapper">
                    ...
                    <input type={"checkbox"} checked={state.enterChecked}
                           name={'enterChecked'}
                    ...
                    <span> 압력 (Pa / m) </span>
                    <input
                        className={`checkBoxInputenter ${state.enterDisable ? 'enterDisabled' : ''}`}
                        value={state.enter} onChange={handleChange}
                        disabled={state.enterDisable} name={"enter"}
                        onKeyDown={handleChange} ref={useRefs.enter} required/>
                </div>
                <br/>
                <div className="checkbox-wrapper">
                    {/*체크 해제 되면 리스트에 나열된 Result 를 모두 삭제*/}
                    <input type={"checkbox"} checked={state.speedChecked}
                           name={'speedChecked'}
                           onChange={handleCheckboxChange}/>
                    <span>속도 (m / s )</span>
                    ...
                    <input
                        className={`checkBoxInputspeed ${state.speedDisable ? 'speedDisabled' : ''}`}
                        value={state.speed} onChange={handleChange}
                        disabled={state.speedDisable} name={"speed"}
                ...
                <div className={"DuctInputSelectClass"}>
                    <div className={"SelectPart"}>
                        <label>
                            ...
                            <select name={"ducts"} onChange={handleChange}
                                    value={state.ducts}>
                        ...
                        <label className="CMHInput">
                            ...
                            <input name={"windVolume"} value={state.windVolume}
                                   onChange={handleChange}
                                   onKeyDown={handleChange}
                                   ref={useRefs.windVolume} required/>
                        </label>
                    </div>
                    <div>
                        {/*g1 이 on 일 경우에는 자동으로  "" 빈문자열 대입*/}
                        ...
                        <span>H(mm)</span>
                        <input name={"firstH"}
                               value={state.ducts === "원형덕트" ? " " : state.firstH}
                               onChange={handleChange} onKeyDown={handleChange}
                               disabled={state.ducts === "원형덕트"}
                               ref={useRefs.firstH} required/>
                        <span>{state.ducts === "원형덕트" ? "D(mm)" : "W(mm)"}</span>
                        <input readOnly
                               value={state.ducts === "원형덕트" ? state.firstD : state.firstW}/>
                        <span>P(Pa/m)</span>
                    ...
                    <div>
                        ...
                        <span>H(mm)</span>
                        <input name={"secondH"}
                               value={state.ducts === "원형덕트" ? " " : state.secondH}
                               onChange={handleChange}
                               onKeyDown={handleChange}
                               disabled={state.ducts === "원형덕트"}
                               ref={useRefs.secondH} required/>
                        {/*g1 이 on 일 경우에는 자동으로  "" 빈문자열 대입*/}
                        {/*10000000이상 0 미만이면 "" 빈문자열 */}
                        <span>{state.ducts === "원형덕트" ? "D(mm)" : "W(mm)"}</span>
                        <input
                            name={state.ducts === "원형덕트" ? "secondD" : "secondW"}
                            value={state.ducts === "원형덕트" ? state.secondD : state.secondW}
                            onChange={handleChange}
                            onKeyDown={handleChange} ref={useRefs.secondW}
                            required/>
                        <span>P(Pa/m)</span>
                        <input className={"paInputBox"} readOnly
                               value={state.secondP}/>
